...
void USART_SendData(USART_Handle_t *pUSARTHandle, uint8_t *pTxBuffer, uint32_t Length)
{
    ...
    for(uint32_t i = 0 ; i < Length; i++)
    {
        ...
        if(pUSARTHandle->USART_Config.USART_WordLength == USART_WORDLEN_9BITS)
        {
            ...
            pUSARTHandle->pUSARTx->RDR = (*pdata & (uint16_t)0x01FF);
        ...
        {
            /* 8bit data transfer */
            pUSARTHandle->pUSARTx->RDR = (*pTxBuffer  & (uint8_t)0xFF);
...
uint8_t USART_SendDataInterrupt(USART_Handle_t *pUSARTHandle,uint8_t *pTxBuffer, uint32_t Length)
{
    ...
    if(txstate != USART_BUSY_IN_TX)
    {
        ...
        pUSARTHandle->pUSARTx->CR1 |= (1 << USART_CR1_TXFNFIE);
...
uint8_t USART_ReceiveDataInterrupt(USART_Handle_t *pUSARTHandle, uint8_t *pRxBuffer, uint32_t Length)
{
    ...
    if(rxstate != USART_BUSY_IN_RX)
    {
        ...
        pUSARTHandle->pUSARTx->CR1 |= (1 << USART_CR1_RXFNEIE);
    }
...
void USART_IRQHandling(USART_Handle_t *pUSARTHandle)
{
    ...
    temp1 = pUSARTHandle->pUSARTx->ISR & (1 << USART_ISR_TXFNE);

    /* Checking state for TXEIE bit in the CR1 */
    temp2 = pUSARTHandle->pUSARTx->CR1 & (1 << USART_CR1_TXFNFIE);
    ...
    if(temp1 && temp2)
    {
        ...
        if(pUSARTHandle->TxBusyState == USART_BUSY_IN_TX)
        {
            ...
            if(pUSARTHandle->TxLen > 0)
            {
                ...
                {
                    /* 8bit data transfer */
                    pUSARTHandle->pUSARTx->RDR = (*pUSARTHandle->pTxBuffer & (uint8_t)0xFF);
    ...
    temp1 = pUSARTHandle->pUSARTx->ISR & ( 1 << USART_ISR_RXFNE);
    temp2 = pUSARTHandle->pUSARTx->CR1 & ( 1 << USART_CR1_RXFNEIE);
    ...
    temp2 = pUSARTHandle->pUSARTx->CR1 & USART_CR1_RXFNEIE;
    if(temp1  && temp2)
    {
